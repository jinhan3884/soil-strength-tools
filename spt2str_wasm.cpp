...
struct ProfilePoint {
    ...
    int type = 0; // 1 = Clay, 0 = Sand (user input; majority vote for layers)
};
    ...
    void CalculateLayerParameters() {
        ...
        for (size_t i = 0; i < sliced_spt_data.size(); ++i) {
            ...
            if (layer_idx != -1 && layer_idx < (int)layer_results.size()) {
                ...
                if (sliced_spt_data[i].type == 1) layer_results[layer_idx].clay_count++;
        ...
        for (auto& lr : layer_results) {
            if (lr.count > 0) {
                ...
                lr.type = (lr.clay_count * 2 >= lr.count) ? 1 : 0; // majority (clay wins ties)
            }
        }
        for (size_t i = 0; i < layer_results.size(); ++i) {
            if (layer_results[i].count == 0) {
                ...
                if (i > 0) layer_results[i].type = layer_results[i - 1].type;
                else if (i < layer_results.size() - 1) layer_results[i].type = layer_results[i + 1].type;
            }
        }
    }
    ...
    auto emit = [&](const std::vector<ProfilePoint>& v) {
        ...
        for (size_t i = 0; i < v.size(); ++i) {
            ...
            ss << "{\"depth\":" << safe(p.depth) << ",\"c\":" << safe(p.c) << ",\"f\":" << safe(p.f)
               << ",\"g\":" << safe(p.g) << ",\"E\":" << safe(p.E) << ",\"type\":" << p.type << "}";
        }
        ss << "]";
    };
    ss << "{\"layers\":";
